...
class AlphaShearletTransform:
    ...
    **Keyword parameters**
    ...
    @property
    def adjoint_coeff(self):
        r"""
        A list (or a generator) of the (space-side, non-normalized)
        alpha-shearlets associated to the adjoint transform.
        ...
        im = np.zeros((self.height, self.width))
        im[self.height // 2, self.width // 2] = 1
        im = im.reshape(1,self.height,self.width)
        im = im.repeat(self.shearlets.shape[0],axis=0)
        if self.is_real:
            return np.real(self.adjoint_scales(im, do_norm=False))
        else:
            return self.adjoint_scales(im, do_norm=False)
    ...
    def adjoint_scales(self, coeffs, spectrograms=None, do_norm=True):
        r"""
        This method computes and returns the adjoint operator coefficicents 
        to the operator computed by the method :func:`transform`. Since the
        :func:`shearlets` method returns the coefficients associated to the
        shearlet system, this means that :func:`adjoint_coeff` is the methods
        that returns the coefficients associated *synthesis operator*.
        ...
        **Return value**
        ...
        # to determine the correct 'dtype'?
        # result = np.zeros((self.height, self.width), dtype=coeffs[0].dtype)
        result = np.zeros(coeffs.shape, dtype='complex128')
        ...
        if self.is_subsampled:
            ...
            for i, (norm, coeff, spect) in enumerate(zip(self.space_norms,
                ...
                # result += spect * my_fft_shift(fft2(coeff))
                if do_norm:
                    result[i] = ifft2(my_ifft_shift(((spect / norm) *
                               my_fft_shift(self._fft(-1, coeff)))))
                else:
                    result[i] = ifft2(my_ifft_shift(spect * 
                               my_fft_shift(self._fft(-1, coeff))))
    ...
    def inverse_scales(self,
                          ...
                          do_norm=True):
        r"""
        Computes the inverse alpha-shearlet transform coefficients.

        Precisely, this method computes the **pseudo-inverse** of the
        alpha-shearlet transform coefficients. This is the same as first 
        projecting onto the range of the alpha-shearlet transform and then
        inverting.
        ...
        In particular, we have the following::
            ...
            >>> im = np.random.random((512, 512))
            >>> coeff = my_trafo.transform(im)
            >>> np.allclose(im, my_trafo.inverse_coeff(coeff).sum(0))
            True
        ...
        if self.is_parseval:
            #result = self.adjoint_scales(coeffs, do_norm=do_norm)
            result = self.adjoint_scales(renormed_coeffs, do_norm=False)
        elif self.is_subsampled:
            result = self.adjoint_scales(renormed_coeffs,
                                            self.dual_spects,
                                            do_norm=False)
        else:
            ...
            if inverse_spects is None:
                ...
                for i, (norm, coef, spec) in enumerate(zip(self.space_norms,
                    ...
                    coeff_f = my_fft_shift(self._fft(i, coef))
                    if do_norm:
                        ne.evaluate('spec * norm * coeff_f / dual_w',
                                    out=result[i])
                    else:
                        ne.evaluate('spec * coeff_f / dual_w',
                                    out=result[i])
                    result[i] = self._ifft(-1, my_ifft_shift(result[i]))
            else:
                for i, (norm, coeff, inv_spec) in enumerate(zip(self.space_norms,
                                                 coeffs,
                                                 inverse_spects)):
                    # coeff_f = my_fft_shift(self._fft(-1, coeff))
                    coeff_f = self._fft(-1, coeff)
                    ...
                    # result += ne.evaluate('inv_spec * coeff_f')
                    if do_norm:
                        ne.evaluate('inv_spec * coeff_f * norm',
                                    out=result[i])
                    else:
                        ne.evaluate('inv_spec * coeff_f', out=result[i])
                    result[i] = self._ifft(-1, result[i])
        if real:
            result = np.real(result)
        return result
